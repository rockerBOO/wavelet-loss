...
def visualize_losses(losses, component_losses, save_path=None, image_hashes=None):
    ...
    for bar, value in zip(bars, level_losses):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(level_losses)*0.01, 
    ...
    bars = plt.bar(coeff_totals.keys(), coeff_totals.values(), 
                   color=['orange', 'green', 'red', 'purple'], alpha=0.8)
    plt.xlabel('Coefficient Type')
    plt.ylabel('Total Loss')
    plt.grid(True, alpha=0.3)
    # Add value labels
    for bar, (coeff, value) in zip(bars, coeff_totals.items()):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(coeff_totals.values())*0.01, 
                f'{value:.4f}', ha='center', va='bottom', fontsize=9)
    ...
    bars = plt.bar(type_totals.keys(), type_totals.values(), 
    ...
    for bar, (loss_type, value) in zip(bars, type_totals.items()):
        plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(type_totals.values())*0.01, 
                f'{value:.4f}', ha='center', va='bottom', fontsize=9)
    ...
    for i, coeff in enumerate(coeffs):
        for j, level in enumerate(levels):
            ...
            total = sum(v for k, v in component_losses.items() 
                       if k.startswith(f'{coeff}{level}_'))
    ...
    for level in range(len(losses)):
        level_data[level] = {}
        for loss_type in loss_types:
            level_data[level][loss_type] = sum(v for k, v in component_losses.items() 
                                             if k.endswith(f'{loss_type}_loss') and f'{level}_' in k)
    ...
    for i, loss_type in enumerate(loss_types):
        ...
        plt.bar(range(len(losses)), values, bottom=bottoms, 
                label=loss_type.capitalize(), color=colors[i], alpha=0.8)
    ...
    for bar, value in zip(bars, comp_values):
        plt.text(bar.get_width() + max(comp_values)*0.01, bar.get_y() + bar.get_height()/2, 
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Compute Wavelet Loss between two images using VAE latent representations')
    ...
    parser.add_argument('--transform-type', type=str, default='dwt', 
                        choices=['dwt', 'swt', 'qwt'], 
    ...
    parser.add_argument('--save-plot', type=str, 
                        help='Path to save loss visualization plot')
    parser.add_argument('--output-dir', type=str, default='wavelet_loss_results',
    ...
    results_filename = f"wavelet_loss_results_{args.transform_type}_{args.wavelet}_L{args.level}_{image1_hash}_{image2_hash}.txt"
